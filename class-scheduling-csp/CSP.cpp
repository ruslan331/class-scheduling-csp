    ...
    for (const Day& day : days) {
        for (size_t lesson = 1; lesson <= LESSONS_PER_DAY; ++lesson) {
            ...
            for (const Group& group : groups) {
                variables_storage.push_back(Variable{ num, Time{day, lesson}, group });
                free_variables.insert(&variables_storage.back());
                ++num;
            }
            int finish = num - 1;
